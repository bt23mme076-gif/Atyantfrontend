...
// ─── My Sessions ─────────────────────────────────────────────────────────────
const SERVICE_META = {
  "text-qa": { label: "Text Q&A", Icon: MessageSquare },
  "audio-call": { label: "Audio Call", Icon: Headphones },
  "video-call": { label: "Video Call", Icon: Video },
  "resume-review": { label: "Resume Review", Icon: FileText },
};
...
function PastSessionInsights({ sessionId, pipelineStatus, isMentorView, onNavigate }) {
  ...
  const statusInfo = {
    completed: { color: C.green, Icon: CheckCircle2, text: "Session insights ready" },
    processing: { color: C.accentText, Icon: Clock, text: "Insights being generated…" },
    failed: { color: "#F87171", Icon: AlertTriangle, text: "Insight generation failed" },
    no_audio: { color: "#F59E0B", Icon: Mic, text: "No audio captured" },
  }[pipelineStatus] || { color: C.textMuted, Icon: ClipboardList, text: "Session completed" };
  ...
  return (
    <div style={{ marginTop: "1.1rem" }}>
      ...
      >
        ...
        <ChevronDown size={14} color={C.textMuted} style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
      </button>
...
function SessionDetailCard({ s, isUpcoming, onNavigate }) {
  ...
  const svc = SERVICE_META[s.serviceId] || { label: s.topic || "Session", Icon: Sparkles };
      ...
      <div style={{ padding: "1.3rem 1.4rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: "0.78rem", color: C.textSub, marginTop: 2, display: "flex", alignItems: "center", gap: 5 }}>
              {isMentorView && <span style={{ color: C.accentText, fontWeight: 600 }}>Student ·</span>}<svc.Icon size={12} /> {svc.label}
            </div>
          </div>
...
function SavedAnswerCard({ a, onRemove }) {
  ...
  return (
    <div
      ...
      onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 10px 26px -12px rgba(0,0,0,0.28)"; e.currentTarget.style.borderColor = color + "66"; }}
      onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 1px 2px rgba(0,0,0,0.04)"; e.currentTarget.style.borderColor = color + "33"; }}
    >
    ...
    <>
      <div style={{ background: C.bg, minHeight: "100dvh", display: "flex", fontFamily: "'Satoshi',-apple-system,sans-serif", color: C.text }}>
        ...
        <div style={{ position: "relative", flex: 1, overflow: activePage === "ask" ? "visible" : "hidden", height: activePage === "ask" ? "auto" : "100dvh", minHeight: activePage === "ask" ? "100dvh" : undefined, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ position: "relative", zIndex: 1, flex: 1, overflow: activePage === "ask" ? "visible" : (["clarity", "chat"].includes(activePage) ? "hidden" : "auto") }}>
